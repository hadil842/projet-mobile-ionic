...
export class LoginComponent  implements OnInit {
  ...
  async loginclient():Promise<void>{
    ...
    if(await this.client.loginclient(requete)==200){this.messageSucces="Connexion avec Succes";}
  ...
  async loginfreelancer():Promise<void>{
   let requete=JSON.stringify({prenom:this.prenomuser,password:this.password})
    if(await this.freelancer.loginfreelancer(requete)==200){this.messageSucces="Connexion avec Succes";}
